...
if (hasError) {
    showAlert("Atenção", stringsErrorInvalidData, "warning", "Ok")
    return;
  } else {
    ...
    if (final_def_pierce >= 100) {
      ...
      showAlert('Limite de Ignorar Defesa',stringsErrorDef, 'error');
...
async function applyReverseCalculation() {
    ...
    if (!damageInput) {
        showAlert('Atenção', stringsErrorMissingDmg, 'error');
        return;
    }
    ...
    if (simulatedDamage <= 0 || isNaN(simulatedDamage)) {
        ...
        showAlert('Erro', stringsErrorInvalidDmg, 'error');
        return;
    }
    ...
    Toast.fire({ icon: 'success', title: `Dano Base obtido: ${finalResult}` });
}
