...
function renderGrid(containerId, activeMap, allowedCategories) {
    ...
    const groups = {
        general: { title: "Buffs Gerais", items: [] },
        character: { title: "Buffs de Personagens", items: [] },
        pet: { title: "Buffs de Pet", items: [] },
        defense: { title: "Debuffs de Quebrar Defesa", items: [] },
        resist: { title: "Resistências do Inimigo", items: [] }
    };
...
function drawSummaryTable() {
    ...
    if (debuffsContainer && debuffsContainer.innerHTML.trim() === '') {
        ...
        <table class="buff-summary-table">
            <tbody>
                <tr>
                    <td style="color: #7ffa4eff;">Quebra de Defesa</td>
...
window.forceSetBuffs = function(buffsArray, debuffsArray, shouldCalculate = false) {
    if (!window.buffsData) {
        console.warn("Aguardando buffsData para carregar histórico...");
    ...
    try {
        ...
        console.error("Erro ao importar buffs externos", e);
        window.activeBuffs = new Map();
        window.activeDebuffs = new Map();
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(clearBuffsBtn) clearBuffsBtn.addEventListener('click', (e) => {
        ...
        if(typeof askConfirm === 'function') {
            askConfirm('Limpar Buffs', 'Remover buffs e resistências?', () => {
    ...
    if(clearDebuffsBtn) clearDebuffsBtn.addEventListener('click', (e) => {
        e.preventDefault();
        if(typeof askConfirm === 'function') {
            askConfirm('Limpar Debuffs', 'Remover todos os debuffs?', () => {
