...
function renderHistory() {
    ...
    [...historyData].reverse().forEach((item) => {
        ...
        const favTitle = isFav ? "Desmarcar favorito" : "Marcar como favorito";
                    ...
                    <div class="response-list-item-content">
                      <div class="column-left">
                        ...
                        <div class="stat-group result-group">
                            <div class="response-list-item-value">${stringsNormalHitDamage}: <span class="yellow">${Number(item.ui_final_dmg).toLocaleString('pt-BR')}</span></div>
                            <div class="response-list-item-value">${stringsCriticalHitDamage}: <span class="yellow">${Number(item.ui_corrected_final_dmg).toLocaleString('pt-BR')}</span></div>
                            <div class="response-list-item-value">${stringsAverageDamage}: <span class="yellow">${Number(item.avgCritDmg).toLocaleString('pt-BR')}</span></div>
                        </div>
                      </div>
                      ...
                      <div class="column-right">
                        ...
                        <div class="stat-group result-group">
                            <div class="response-list-item-value">${stringsNormalHitDamage} (costas): <span class="yellow">${Number(item.ui_back_normal).toLocaleString('pt-BR')}</span></div>
                            <div class="response-list-item-value">${stringsCriticalHitDamage} (costas): <span class="yellow">${Number(item.ui_back_critical).toLocaleString('pt-BR')}</span></div>
                            <div class="response-list-item-value">${stringsAverageDamage} (costas): <span class="yellow">${Number(item.ui_back_average).toLocaleString('pt-BR')}</span></div>
                        </div>
                      </div>
                    </div>
                </div>
...
function toggleHistorySelection(id, checkbox) {
    if (checkbox.checked) {
        if (selectedHistoryIds.size >= 2) {
            ...
            showAlert(
                'Limite atingido', 
                'Você só pode selecionar 2 itens para comparar.', 
                'warning'
            );
...
function compareSelected() {
    ...
    const getStatRow = (label, key, suffix = '', extraMetaKey = null) => {
        ...
        const formatVal = (val) => !isNaN(parseFloat(val)) ? parseFloat(val).toLocaleString('pt-BR') : val;
    ...
    const detailsHtml = `
        ${getStatRow('Ataque', 'ATK')}
        ${getStatRow('Ataque especial', 'sATK')}
        ${getStatRow('Chance de crítico', 'crit_r', '%', 'savedCritExtra')}
        ${getStatRow('Dano crítico', 'crit_d', '%', 'savedCdmg')}
        ${getStatRow('Chance de Lança Infernal', 'hs_chance', '%')}
        ${getStatRow('Dano de Lança Infernal', 'hs_dmg')}
        
        ${getStatRow('Debuff contaminação', 'harrier_debuff', '%')}
        ${getStatRow('Resist. contaminação', 'harrier_resist', '%')}
        
        ${getStatRow('Ataque pelas costas', 'b_atk', '%')}
        ${getStatRow('Quebra de defesa', 'def_pierce', '%', 'savedDefPierce')}
        ${getStatRow('Total de Buffs', 'calculated_total_buffs', '%')}
        ${getStatRow('Dano base', 'skill_multi')}
    `;
    ...
    modalText.innerHTML = `
        <div class="modal-header">
            <div class="modal-name">Comparativo de Builds</div>
        </div>
        ...
        <div class="modal-body">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="compare-row-label">
                            Hit Crítico<br>
                            <span style="font-size:0.8em; font-weight:normal; color:#aaa">(Hit Imediato)</span>
                        </td>
                        <td>${rowCrit.htmlA}</td>
                        <td>${rowCrit.htmlB}</td>
                    </tr>
                    <tr>
                        <td class="compare-row-label">
                            Dano Médio<br>
                            <span style="font-size:0.8em; font-weight:normal; color:#aaa">(Longo Prazo)</span>
                        </td>
                        <td>${rowAvg.htmlA}</td>
                        <td>${rowAvg.htmlB}</td>
                    </tr>
            ...
            <div style="margin-top: 15px; text-align: center; font-size: 0.9em; color: #aaa;">
                <span class="diff-badge" style="font-size: 12px;">▲ X%</span> Indica a porcentagem de dano que a build mais forte causa em relação à mais fraca.<br>
                A comparação é feita na ordem em que foram selecionadas.
            </div>
        </div>
    `;
...
function clearHistory() {
    askConfirm(
        "Limpar histórico?", 
        "Todos os cálculos (exceto Favoritos) serão excluídos permanentemente.", 
        () => {
            ...
            if (historyData.length > 0) {
                 showToast("Histórico limpo! (Favoritos mantidos)");
            } else {
...
function loadHistoryItem(id) {
    ...
    askConfirm(
        'Carregar Cálculo?',
        'Os dados atuais da tela serão substituídos pelas informações deste salvo.',
        () => {
